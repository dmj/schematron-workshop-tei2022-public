<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
  ...
  <pattern abstract="true" id="referencing-string-register-link">
    <rule context="tei:rs[@type = '$type'][starts-with(@ref, 'register:')]">
      ...
      <assert test="document($registerUriResolved, .)/self::$elementName">
        A <name/> element of type '<value-of select="'$type'"/>' must reference a <value-of select="'$elementName'"/> element. The reference '<value-of select="@ref"/>' points to something different.
      </assert>
    </rule>
  </pattern>

  <pattern is-a="referencing-string-register-link">
    <param name="type" value="work"/>
    <param name="elementName" value="tei:bibl"/>
  </pattern>

  <pattern is-a="referencing-string-register-link">
    <param name="type" value="organization"/>
    <param name="elementName" value="tei:org"/>
  </pattern>
  ...
  <pattern is-a="referencing-string-register-link">
    <param name="type" value="place"/>
    <param name="elementName" value="tei:place"/>
  </pattern>
  ...
  <pattern id="register-entry">
    <rule abstract="true" id="require-id">
      <assert test="@xml:id">
        Every <name/> element must have an @xml:id attribute.
      </assert>
    </rule>
    <rule context="tei:bibl | tei:org | tei:person | tei:place">
      <extends rule="require-id"/>
    </rule>
  </pattern>
